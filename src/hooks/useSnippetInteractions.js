...
export const useSnippetInteractions = (currentUser, snippets, setSnippets) => {
  ...
  const { addComment, updateSnippetReaction, deleteComment } =
  ...
  const handleCommentSubmit = useCallback(
    async (snippetId) => {
      ...
      try {
        setSubmittingComment((prevState) => ({
          ...prevState,
          [snippetId]: true,
        }));
        ...
        const updatedResult = {
          ...snippets.find((result) => result.id === snippetId),
        };
  ...
  const handleReaction = useCallback(
    async (snippetId, isLike) => {
      if (!currentUser) {
        toast.showWarning("Please log in to react to snippets");
        return;
      }

      const existingReaction = snippets.find(
        (result) => result.id === snippetId
        ...
        if (existingReaction) {
          if (existingReaction === (isLike ? "like" : "dislike")) {
            if (isLike) {
              ...
              updatedResult.dislikes = Math.max(
                (updatedResult.dislikes || 0) - 1,
                0
              );
            }
            ...
            updatedResult.userReactions[currentUser.uid] = isLike
              ? "like"
              : "dislike";
          }
        } else {
  ...
  const handleDeleteComment = useCallback(
    async (snippetId, commentId) => {
      ...
      try {
        ...
        if (snippetDoc) {
          setSnippets((prev) =>
            prev.map((s) => (s.id === snippetId ? snippetDoc : s))
          );
        }
      }
    },
