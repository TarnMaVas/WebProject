...
export const useUserProfile = () => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        setProfileData({
          ...
          createdAt: auth.currentUser?.metadata?.creationTime ? new Date(auth.currentUser.metadata.creationTime) : null
        });
      }
    } catch (err) {
  ...
  const fetchUserStats = async (userId) => {
    try {
      ...
      const favoritesCount = favoritesDoc.exists() ? 
        (favoritesDoc.data().snippetIds || []).length : 0;
        let commentsCount = 0;
      ...
      setStats({
        ...
        favoritesCount,
        commentsCount,
        joinedDate: auth.currentUser?.metadata?.creationTime ? new Date(auth.currentUser.metadata.creationTime) : null
      });
    } catch (err) {
  ...
  const updateProfileData = useCallback(async (data) => {
    try {
      ...
      const optionalFields = ['location', 'website', 'github', 'twitter', 'linkedin'];
  ...
  }, [currentUser, toast]);  const uploadUserAvatar = useCallback(async (file, options = {}) => {
    try {
      setLoading(true);
      setError(null);
